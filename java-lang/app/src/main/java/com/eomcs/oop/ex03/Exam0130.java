...
// JVM 을 실행하는 동안 클래스가 로딩되면 중복 로딩되지 않는다.
// 클래스 변수는 클래스가 로딩될 때 자동으로 생성되기 때문에
// 클래스에 대해 딱 한번 생성된다
//
...
//     예) System.out.println(A.v1);  => 스태틱 변수 사용 
//     예) Integer.parseInt(..);  => 스태틱 메서드 사용
